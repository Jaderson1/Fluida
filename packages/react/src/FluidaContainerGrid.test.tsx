...
describe('FluidaContainerGrid', () => {
  it('renders its children', () => {
    installMockResizeObserver();

    const { getByText } = render(
      <FluidaContainerGrid itemCount={1}>
        <span>cell</span>
      </FluidaContainerGrid>,
    );
  ...
  it('forwards a ref to the underlying div, while still measuring it internally', () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={1} ref={ref} data-testid="grid">
        <span>cell</span>
      </FluidaContainerGrid>,
    );
  ...
  it('forwards standard div props', () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={1} data-testid="grid" className="custom-class">
        <span>cell</span>
      </FluidaContainerGrid>,
    );
  ...
  it("applies fit's square cell sizing after a real measurement", () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={4} strategy="fit" gap={0} data-testid="grid">
        <span>1</span>
        <span>2</span>
        <span>3</span>
        <span>4</span>
  ...
  it('throws for an itemCount below 1, via the same FluidaConfigError Core already uses', () => {
    ...
    expect(() => {
      render(
        <FluidaContainerGrid itemCount={0} data-testid="grid">
          <span>cell</span>
        </FluidaContainerGrid>,
      );
    }).toThrow();
  ...
  describe('minItemWidth', () => {
    ...
    it('reduces the column count in a narrow container compared to the same container without minItemWidth', () => {
      ...
      const withoutConstraint = render(
        <FluidaContainerGrid itemCount={4} gap={0} strategy="fill" data-testid="grid-a">
          <span>1</span>
          <span>2</span>
          <span>3</span>
          <span>4</span>
    ...
    it('propagates FluidaConfigError for an invalid minItemWidth', () => {
      ...
      expect(() => {
        render(
          <FluidaContainerGrid itemCount={4} minItemWidth={0} data-testid="grid">
            <span>1</span>
          </FluidaContainerGrid>,
        );
      }).toThrow();
...
describe('height safety (regression)', () => {
  it('never sets height: 100% — the property that caused the deadlock is gone entirely', () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={1} data-testid="grid">
        <span>cell</span>
      </FluidaContainerGrid>,
    );
  ...
  it("lets a consumer's own style.height override the default minHeight", () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={1} data-testid="grid" style={{ height: '500px' }}>
        <span>cell</span>
      </FluidaContainerGrid>,
    );
  ...
  it("lets a consumer's own style.minHeight override the default", () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={1} data-testid="grid" style={{ minHeight: '10px' }}>
        <span>cell</span>
      </FluidaContainerGrid>,
    );
...
describe('itemCount vs. rendered children (development warning)', () => {
  it('warns in development when itemCount does not match the number of children', () => {
    ...
    render(
      <FluidaContainerGrid itemCount={3}>
        <span>1</span>
        <span>2</span>
      </FluidaContainerGrid>,
    );
  ...
  it('does not warn when itemCount matches the number of children', () => {
    ...
    render(
      <FluidaContainerGrid itemCount={2}>
        <span>1</span>
        <span>2</span>
      </FluidaContainerGrid>,
    );
  ...
  it('does not throw when itemCount and children disagree', () => {
    ...
    expect(() => {
      render(
        <FluidaContainerGrid itemCount={5}>
          <span>1</span>
        </FluidaContainerGrid>,
      );
    }).not.toThrow();
  ...
  describe('autoHeight', () => {
    ...
    it('throws the same FluidaConfigError as Core for fit + autoHeight without minItemWidth', () => {
      ...
      expect(() => {
        render(
          <FluidaContainerGrid itemCount={6} strategy="fit" autoHeight data-testid="grid">
            {Array.from({ length: 6 }, (_, i) => (
              <span key={i}>{i}</span>
            ))}
          </FluidaContainerGrid>,
...
describe('accessibility', () => {
  it('sets no role or aria-label unless the consumer provides one', () => {
    ...
    const { getByTestId } = render(
      <FluidaContainerGrid itemCount={2} gap={0} strategy="fit" data-testid="grid">
        <span>1</span>
        <span>2</span>
      </FluidaContainerGrid>,
    );
  ...
  it('renders children in their given order with no tabIndex of its own, for normal keyboard focus order', () => {
    ...
    const { getAllByRole } = render(
      <FluidaContainerGrid itemCount={3} gap={0} strategy="fit">
...
describe('itemCount is optional, inferred from children when omitted', () => {
  it('infers itemCount from Children.count when omitted entirely', () => {
    installMockResizeObserver();

    const { getByTestId } = render(
      <FluidaContainerGrid strategy="fit" gap={0} data-testid="grid">
        <span>1</span>
        <span>2</span>
        <span>3</span>
      </FluidaContainerGrid>,
    ...
    act(() => {
      getLiveObserverFor(getByTestId('grid'))?.trigger(300, 300);
    });

    // Same result an explicit itemCount={3} would produce — confirms
    // the inferred count actually reaches the layout computation, not
    // just that nothing throws.
    const inferred = getByTestId('grid').style.gridTemplateColumns;

    cleanup();
    installMockResizeObserver();
    const { getByTestId: getByTestIdExplicit } = render(
      <FluidaContainerGrid itemCount={3} strategy="fit" gap={0} data-testid="grid">
        <span>1</span>
        <span>2</span>
        <span>3</span>
      </FluidaContainerGrid>,
    ...
    act(() => {
      getLiveObserverFor(getByTestIdExplicit('grid'))?.trigger(300, 300);
    });

    expect(inferred).toBe(getByTestIdExplicit('grid').style.gridTemplateColumns);
  });

  it('emits no development warning when itemCount is inferred, even though nothing was passed to compare against', () => {
    installMockResizeObserver();
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

    render(
      <FluidaContainerGrid strategy="fit" gap={0}>
        <span>1</span>
        <span>2</span>
      </FluidaContainerGrid>,
    );
  ...
  it('an explicit itemCount still overrides the inferred count, and still warns on mismatch', () => {
    installMockResizeObserver();
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

    render(
      <FluidaContainerGrid itemCount={5} strategy="fit" gap={0}>
        <span>1</span>
        <span>2</span>
      </FluidaContainerGrid>,
    );

    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('itemCount={5}'));
    warnSpy.mockRestore();
  });
});

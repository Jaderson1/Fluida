...
export interface FluidaContainerGridProps extends ComponentPropsWithoutRef<'div'> {
  /**
   * How many children will be rendered. Optional — defaults to
   * `Children.count(children)` when omitted. Pass this explicitly
   * when that count would be wrong for your case: fragments, `null`/
   * conditional children, or any content Children.count miscounts —
   * the explicit value is also what the development-mode mismatch
 ...
 */
export const FluidaContainerGrid = forwardRef<HTMLDivElement, FluidaContainerGridProps>(
  function FluidaContainerGrid(
    {
      itemCount,
      strategy,
      gap,
    ...
    const effectiveItemCount = itemCount ?? renderedChildCount;
    ...
    useEffect(() => {
      // Nothing to warn about when itemCount is inferred — it's
      // renderedChildCount by construction. Only an explicit itemCount
      // can disagree with what actually rendered.
      ...
      console.warn(
        `Fluida: <FluidaContainerGrid itemCount={${itemCount}}> was given ${renderedChildCount} child element(s). ` +
